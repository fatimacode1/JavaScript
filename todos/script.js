...
function addTodo() {
  ...
  newEdit.setAttribute("id", `edit-${counter}`);
...
function editTodo(counter) {
  const targetH4 = document.getElementById(counter);
  const editBtn = document.getElementById(`edit-${counter}`);

  if (editBtn.innerText === "edit") {
    const oldText = targetH4.childNodes[0].nodeValue;

    const editInput = document.createElement("input");
    editInput.value = oldText;

    targetH4.childNodes[0].nodeValue = "";
    targetH4.insertBefore(editInput, editBtn);

    editBtn.innerText = "save";
  } else {
    const editInput = targetH4.querySelector("input");
    const newText = editInput.value;

    targetH4.removeChild(editInput);
